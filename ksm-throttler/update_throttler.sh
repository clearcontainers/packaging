...
GENERATED_FILES=(_service cc-ksm-throttler.spec cc-ksm-throttler.dsc debian.control debian.rules)
...
PROJECT_REPO=${PROJECT_REPO:-home:clearcontainers:clear-containers-3-staging/cc-ksm-throttler}
...
function template()
{
	...
	cc-ksm-throttler.spec-template > cc-ksm-throttler.spec

    sed -e "s/@VERSION@/$VERSION/g" \
        -e "s/@HASH@/${HASH:0:7}/g"\
        -e "s/@RELEASE@/$RELEASE/g" \
	cc-ksm-throttler.dsc-template > cc-ksm-throttler.dsc
